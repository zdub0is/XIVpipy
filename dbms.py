...
#{"_id":"2","Name":"Fire Shard","Recipes":"No recipes","shopPrice":{"$numberInt":"9"},"shopSellPrice":{"$numberInt":"0"},"containsUntradeable":false,"profitMargin":{"$numberInt":"1"}}
  ...
  def update(self, id, update):
    try:
      self.conn.find_one_and_update(id,
                                    update,
  ...
  def ping(self):
    if self.db.command("ping")['ok'] == 1:
      ...
      cfg.ERRLOG.append(
        'PyMongo is not responding. Please check the connection.')
  ...
  def export(self, worldID, recipeExport):
    ...
    print(cfg.WARNING + "Beginning export process " + exportType +
          ", please wait..." + cfg.N)
    ...
    if recipeExport:
      ...
      }).sort("marketboard." + str(worldID) + ".shortProfitIndex",
              -1)  #find based on worldID called
    else:
      items = self.conn.find({
        "$and": [{
          "Recipes": "No recipes"
      ...
      }).sort("marketboard." + str(worldID) + ".shortProfitIndex", -1)
    try:
      for item in tqdm(items):
        temp = {
          "name": item["Name"],
          "profitIndex": item["marketboard"][str(worldID)]["shortProfitIndex"],
          "price": item["marketboard"][str(worldID)]["price"]
        }  #make part of temp
          ...
          for j in item["Recipes"].values():
            ...
            for x, y in j["ingredients"].items():
              ...
              if r is None:
                ...
                continue
              if "cost" in r:
                rec[-1]["Ingredients"].append({
                  "name":
                  r["Name"],
                  "amount":
                  y,
                  "price": (r["marketboard"][str(worldID)]["price"] * y),
                  "cost":
                  r["marketboard"][str(worldID)]["cost"]
                })
              else:
                rec[-1]["Ingredients"].append({
                  "name":
